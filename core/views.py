    ...
    if request.method == 'POST':
        ...
        if password == password2:
            if User.objects.filter(email=email).exists():
                ...
                #log user in, direct to settings page.
                user_login = auth.authenticate(username=username,password=password)
                auth.login(request,user_login)
                
                #create a profile object for the new user.
                user_model = User.objects.get(username=username)
                new_profile = Profile.objects.create(user=user_model,id_user=user_model.id,fullname=fullname)
                new_profile.save()
...
def profile_image(request,pk):
    ...
    return render(request,'profile_images.html',{'post':curr_post})
